...
class TokenIndexer extends Indexer {
    ...
    */
	...
	async validateTokenTx(tx) {
		console.log("validateTokenTx");
		const tx = TX.fromOptions(tx);
		// get tx json with slp data
		const hash = tx.hash();
		console.log("hash", hash);
		const isSlp = await this.slpindex.isSlpTX(hash);
		console.log("isSlp", isSlp);
		if (!isSlp) {
			return false;
		}

		const slpTx = await this.slpindex.getSlpCoinRecords(tx);		
		console.log("slpTx", slpTx);
		const tokenId = slpTx.slpToken.tokenId;
		console.log("tokenId", tokenId);
		const relevantTokens = ["0dbbeca4adccfbf4820c0a748ea5921204bed98e319bbd0879f7010d4297ed31"];
		const hasRelevantToken = relevantTokens.includes(tokenId);
