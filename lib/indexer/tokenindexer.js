...
class TokenIndexer extends Indexer {
    ...
    */
	...
	async validateTokenTx(txInput, meta) {
		...
		let hasTokenPrevout = false;
		if (!isSlp) {
			const prevouts = tx.inputs.map(input => input.prevout);
			if (meta.height === 804826) console.log("prevouts", prevouts);
			for (const prevout of prevouts) {
				const prevoutHash = prevout.hash;
				...
				if (isSlpPrevoutTx) {
					...
					const tokenIds = prevoutSlpCoinRecords.map(records => record.getJSON().tokenId);
		...
		if (meta.height === 804826) console.log("isSlp hasTokenPrevout", isSlp, hasTokenPrevout);

		if (!isSlp && !hasTokenPrevout) {
			return false;
		}
