...
class TokenIndexer extends Indexer {
    ...
    */
	 ...
	 * @returns {Array}
	 */
	async getUtxosByAddress(addrString) {
		const addr = new Address(addrString, this.network);

		const coins = [];

		const memCoins = await this.mempool.getCoinsByAddress(addr);
	...
	async validateTokenTx(txInput) {
		...
		console.log("slpCoinRecords", slpCoinRecords);
		const tokenIds = slpCoinRecords.map(record =>  record.getJSON().tokenId);
		console.log("tokenIds", tokenIds);
		// const relevantTokens = ["0dbbeca4adccfbf4820c0a748ea5921204bed98e319bbd0879f7010d4297ed31"]; // MUSD
		const relevantTokens = ["4075459e0ac841f234bc73fc4fe46fe5490be4ed98bc8ca3f9b898443a5a381a"]; // BUXs
		...
		console.log("hasRelevantToken", hasRelevantToken);
