    ...
    */
    ...
    async unindexBlock(meta, block, view) {
		...
		if (rawBrecord) {
			...
			if (meta.height === 804826) console.log("brecord start end", start, end);
			for (let i = start; i <= end; i++) {
				if (meta.height === 804826) console.log("hashIndexInt", i);
				// delete hash index <-> hash entries
				const hashIndexInt = i;
				const hash = await this.db.get(layout.T.encode(hashIndexInt));
				if (meta.height === 804826) console.log("hash to delete", hash);
				await this.db.del(layout.t.encode(hash));
				if (meta.height === 804826) console.log("hashIndexInt to delete", hashIndexInt);
				...
				if (meta.height === 804826) console.log("addresses to delete", addresses);
				...
				if (hasAddress) {
					if (meta.height === 804826) console.log("deleting height, i", height, i);
					this.del(layout.C.encode(height, i));
					if (meta.height === 804826) console.log("deleting hash", hash);
					this.del(layout.c.encode(hash));
				}
			}

			// delete block record
			if (meta.height === 804826) console.log("deleting height", meta.height);
	...
	async validateTokenTx(txInput, meta) {
		...
		let hasRelevantToken = false;
		for (const relevantTokenId of relevantTokens) {
			const hasToken = tokenIds.includes(relevantTokenId);
			if (hasToken) {
				hasRelevantToken = true;
				break;
			}
		}
		if (meta.height === 804826) console.log("hasRelevantToken", hasRelevantToken);
